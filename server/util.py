...
def classify_image(image_base64_data, file_path=None):
    ...
    for img in imgs:
        ...
        result.append({
            'class': get_class_name(__model.predict(final)[0]),
...
def load_saved_artifacts():
    ...
    with open("./artifacts/celebrity_classifier_class_dictionary.json", "r") as f:
        class_name_to_number = json.load(f)
        class_number_to_name = {v:k for k,v in class_name_to_number.items()}

    global __model
    if __model is None:
        with open('./artifacts/celebrity_classifier_model.pkl', 'rb') as f:
